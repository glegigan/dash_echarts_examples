...
def gen_next_year_data(year=None):
    if not year:
        ...
        if index >= len(years)-1:
            ...
            year = years[index+1]
    return year, [e for e in _rawData[1:] if e[4] == year]
...
layout = html.Div([
    dash_echarts.DashECharts(
        ...
        fun_values=['xAxisFormatter', 'yAxisFormatter', 'series_color'],
        style={
            "width": '100%',
            "height": '95vh',
        },
...
def main():
    ...
    @app.callback(
        ...
        inputs=[
            ...
            Input(component_id="bar-race-button", component_property='n_clicks')
        ],
        state=[
            Input(component_id="echarts", component_property='option'),
        ],
    )
    def update_line_race(n_intervals, btn, opt):
        if n_intervals > 0:
